...
class cource_following_learning_node:
    def __init__(self):
        ...
        self.vel_sub = rospy.Subscriber("/nav_vel", Twist, self.callback_vel, queue_size=10)
    ...
    def loop(self):
        ...
        """
        if self.learning:
            action, loss = self.dl.act_and_trains(self.cv_image , target_action)
            if abs(target_action) < 0.1:
                action_left,  loss_left  = self.dl.act_and_trains(self.cv_left_image , target_action - 0.2)
                action_right, loss_right = self.dl.act_and_trains(self.cv_right_image , target_action + 0.2)
