...
class cource_following_learning_node:
    def __init__(self):
        ...
        self.collect_data = rospy.Service('/collect_data', Trigger, self.collect_data)
        self.save_img_no = 0
    ...
    def loop(self):
        ...
        if self.learning:
            ...
            action, loss = self.dl.make_dataset(imgobj, target_action)
            
            if abs(target_action) < 0.1:
                action_left,  loss_left  = self.dl.trains(imgobj_left, target_action - 0.2)
                action_right, loss_right = self.dl.trains(imgobj_right, target_action + 0.2)
            ...
            self.vel.angular.z = target_action
            self.nav_pub.publish(self.vel)lf.action
